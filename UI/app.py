...
def main():
    ...
    if search_button or (
        search_query and st.session_state.search_query != search_query
    ):
    ...
    if st.session_state.graph_data:
        ...
        with col_graph:
            ...
            try:
                ...
                st.warning(
                    "Попробуйте нажать 'Обновить визуализацию' или перезагрузить страницу."
                )
        ...
        node_options = {
            n.get("id"): f"{n.get('label')} ({n.get('type')})" for n in available_nodes
        }
        ...
        with col_report3:
            st.write("")
            if st.button(
                "Скачать отчет (PDF)", use_container_width=True, type="primary"
            ):
                try:
                    ...
                    edge_types_for_report = (
                        selected_edge_types if selected_edge_types else None
                    )
                    ...
                    st.info(
                        "Убедитесь, что установлен пакет reportlab: pip install reportlab"
                    )
